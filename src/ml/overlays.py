...
def _ensure_overlay_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    default_columns: dict[str, Any] = {
        ...
        "ml_trade_quality_proba": pd.NA,
        "ml_rebalance_proba": pd.NA,
    }
...
def _predict_proba(artifact: dict[str, Any], features_df: pd.DataFrame) -> np.ndarray:
...
def _build_trade_quality_features(signal_df: pd.DataFrame, bucket: str) -> pd.DataFrame:
    ...
    out["vol20"] = _safe_numeric_series(
        signal_df,
        "vol20",
        default=float(out["vol60"].median()) if len(out) else 0.0,
    )
    out["dd_6m"] = _safe_numeric_series(signal_df, "dd_6m")
    out["dd_12m"] = _safe_numeric_series(
        signal_df,
        "dd_12m",
        default=float(out["dd_6m"].median()) if len(out) else 0.0,
    )
...
def apply_trade_quality_overlay(signal_df: pd.DataFrame, bucket: str) -> pd.DataFrame:
    ...
    proba = _predict_proba(artifact, features)

    cfg = get_ml_config()
    centered = 2.0 * (proba - 0.50)
    score_bump = cfg.trade_strength * centered

    out["trade_quality_proba"] = proba
    out["ml_trade_quality_proba"] = proba
    out["trade_quality_score"] = centered
    out["trade_quality_label"] = (proba >= 0.50).astype(int)
    ...
    if bucket_lower == "core" and "selected_core_final" in out.columns:
        ...
        weak_mask = mask_selected & (pd.Series(proba, index=out.index) < 0.35)
        if weak_mask.any() and mask_selected.sum() - weak_mask.sum() >= 4:
            out.loc[weak_mask, "selected_core_final"] = False
            out.loc[weak_mask, "selection_reason"] = "ML_REJECT_LOW_TRADE_QUALITY"
    ...
    if bucket_lower in {"opportunistic", "opp"} and "selected_opp_final" in out.columns:
        ...
        weak_mask = mask_selected & (pd.Series(proba, index=out.index) < 0.38)
...
def apply_rebalance_overlay(df: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
    ...
    out["rebalance_proba"] = proba
    out["ml_rebalance_proba"] = proba
    out["rebalance_score"] = 2.0 * (proba - 0.50)
    out["rebalance_label"] = (proba >= 0.50).astype(int)
    ...
    reject_mask = marginal_trade & (pd.Series(proba, index=out.index) < cfg.rebalance_probability_floor)
    if reject_mask.any():
        out.loc[reject_mask, "execute"] = False
        out.loc[reject_mask, "decision_reason"] = "ML_REJECT_REBALANCE"
...
def get_live_deployment_adjustment(
    ...
    **kwargs,
    ...
    proba = float(_predict_proba(artifact, features)[0])

    cfg = get_ml_config()

    core_multiplier = float(np.interp(proba, [0.0, 1.0], [cfg.core_deployment_floor, cfg.core_deployment_cap]))
    opp_multiplier = float(np.interp(proba, [0.0, 1.0], [cfg.opp_deployment_floor, cfg.opp_deployment_cap]))
    ...
    signal = 2.0 * (proba - 0.50)
    confidence = abs(signal)

    return {
        "deployment_ml_proba": proba,
